...
<style>
    ...
    .tab { padding: 6px 15px; cursor: pointer; border: none; background: transparent; color: white; font-weight: bold; font-size: 12px; }
...
def watch_page_ui(id: str, ep: str = None, type: str = "sub"):
    ...
    return f"""<!DOCTYPE html><html><head><title>Watching {a['title']}</title>{SHARED_CSS}</head><body>{HEADER_HTML}<div class="container"><div class="watch-layout"><div class="main-player"><div class="player-area"><iframe src="{src}" id="player" style="width:100%;height:100%;border:none;" allowfullscreen="true"></iframe></div><div class="controls"><div class="toggle-group"><span>Type:</span><div class="tab-group"><button class="tab {"active" if type=="sub" else ""}" onclick="location.href='/watch-page?id={id}&ep={cur}&type=sub'">SUB</button><button class="tab {"active" if type=="dub" else ""}" onclick="location.href='/watch-page?id={id}&ep={cur}&type=dub'">DUB</button></div></div><div class="toggle-group"><span>Shield:</span><label class="switch"><input type="checkbox" id="shT" checked onchange="upS()"><span class="slider"></span></label></div><div class="toggle-group"><span>Next:</span><label class="switch"><input type="checkbox" id="atN" checked><span class="slider"></span></label></div></div><h1 style="color:var(--primary); margin-top:25px;">{a['title']}</h1><p style="color:#aaa;">{a['description'][:500]}...</p></div><div class="episodes-card"><h3 style="margin-top:0; border-bottom:1px solid #333; padding-bottom:10px; color:var(--primary);">Episodes</h3><div class="ep-grid">{eps}</div></div></div></div><script>const nxt_u = "{nxt}"; const ifr = document.getElementById('player'); function upS() {{ if(document.getElementById('shT').checked) {{ ifr.setAttribute('sandbox', 'allow-scripts allow-same-origin allow-forms'); }} else {{ ifr.removeAttribute('sandbox'); const s = ifr.src; ifr.src = ''; ifr.src = s; }} }} window.addEventListener("message", function(e) {{ let d = e.data; if(typeof d === "string") {{ try {{ d = JSON.parse(d); }} catch(x) {{}} }} if(document.getElementById('atN').checked && (d.event === "complete" || d.type === "complete") && nxt_u) {{ window.location.href = nxt_u; }} }}); upS();</script></body></html>"""
