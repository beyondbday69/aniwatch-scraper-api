...
<style>
    ...
    .tab-group { display: flex; background: #222; border-radius: 5px; overflow: hidden; border: 1px solid #444; }
...
def watch_page_ui(id: str, ep: str = None, type: str = "sub", autoNext: str = "on"):
    ...
    return f"""<!DOCTYPE html><html><head><title>Watching {a['title']}</title>{SHARED_CSS}</head><body>{HEADER_HTML}<div class="container"><div class="watch-layout"><div class="main-player"><div class="player-area"><iframe src="{src}" id="player" style="width:100%;height:100%;border:none;" allowfullscreen="true" sandbox="allow-scripts allow-same-origin allow-forms"></iframe></div><div class="controls"><div class="toggle-group"><span>Type:</span><div class="tab-group"><button class="tab {"active" if type=="sub" else ""}" onclick="location.href='/watch-page?id={id}&ep={cur}&type=sub&autoNext={autoNext}'">SUB</button><button class="tab {"active" if type=="dub" else ""}" onclick="location.href='/watch-page?id={id}&ep={cur}&type=dub&autoNext={autoNext}'">DUB</button></div></div><div class="toggle-group"><span>Auto Next:</span><div class="tab-group"><button class="tab {"active" if autoNext=="on" else ""}" onclick="location.href='/watch-page?id={id}&ep={cur}&type={type}&autoNext=on'">ON</button><button class="tab {"active" if autoNext=="off" else ""}" onclick="location.href='/watch-page?id={id}&ep={cur}&type={type}&autoNext=off'">OFF</button></div></div></div><h1 style="color:var(--primary); margin-top:25px;">{a['title']}</h1><p style="color:#aaa;">{a['description'][:500]}...</p></div><div class="episodes-card"><h3 style="margin-top:0; border-bottom:1px solid #333; padding-bottom:10px; color:var(--primary);">Episodes</h3><div class="ep-grid">{eps}</div></div></div></div><script>const nxt_u = "{nxt}"; window.addEventListener("message", function(e) {{ let d = e.data; if(typeof d === "string") {{ try {{ d = JSON.parse(d); }} catch(x) {{}} }} if("{autoNext}" === "on" && (d.event === "complete" || d.type === "complete") && nxt_u) {{ window.location.href = nxt_u; }} }});</script></body></html>"""
